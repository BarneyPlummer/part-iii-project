...
vector<StationPair> construct_station_pairs(string filename, int tslen);
...
// ---------------------------------------------------------------------- //
vector<StationPair> 
construct_station_pairs(string filename, int tslen)
{
  ...
  for ( unsigned int i=0; i < stationlist.size(); i++ )
    {
      ...
      for ( unsigned int j=i+1; j < stationlist.size(); j++ )
	{
			 ...
			 SacHeader ccorr_header)
{
  ...
  // write SAC file for xcorr with +ve and -ve lags
  Sac sacf;
  ...
  // write SAC file for average of +ve and -ve lags
  sacf.header.b = 0;
  sacf.header.e = (n_pos-1)*sacf.header.delta;
...
// ---------------------------------------------------------------------- //
int 
correlate_station_pair(StationPair sp, int tslen, float delta)
{
  ...
  cout << "Correlating station pair: " << sp.stn1 << " " << sp.stn2 << endl;
  for ( unsigned int i = 0; i < sp.stn1_files.size(); i++ )
    {
      string file1 = sp.stn1_files[i].string();
      string file2 = sp.stn2_files[i].string();
      ...
      else
	{
      ...
      // add the padding (note that this makes the SAC headers inconsistent,
  ...
  output_correlation_files(outfilebase,ccorr_stack,ccorr_header);
...
main(int argc, char *argv[])
{
  ...
  vector<StationPair> station_pairs;
  station_pairs = construct_station_pairs(filelist,ts_len);
  ...
  for ( vector<StationPair>::iterator sp = station_pairs.begin();
	sp != station_pairs.end(); ++sp )
    {
      int stat = correlate_station_pair(*sp,ts_len,delta);
