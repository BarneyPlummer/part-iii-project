...
// Modifications
// Author: Rob Green
// Modifications to run cross-correlation and autocorrelation 
//
			 ...
			 SacHeader ccorr_header, int long_out)
{
  ...
  if ( long_out < 1 )
    ...
    write_sacfile(filenamebase+"_long.SAC",&sacf);
    }

  // Change to shorter length xcorr to output (RG)
  int out_length = 14401; // MUST BE ODD!
  int out_half_length = out_length / 2; // Float round down to an int (7200)
